#!/bin/bash
# Daily Jira sync — runs every morning before work.
# Pulls the open sprint + recently updated tickets into knowledge/jira/.
# Called by cron. Uses a login shell so MCP credentials are available.

set -euo pipefail

CLAUDE="/Users/abhishek.shah/.local/bin/claude"
LOG="$HOME/brain/logs/sync.log"
...
"$CLAUDE" \
  ...
  -p "Read ~/brain/.claude/skills/brain-sync/SKILL.md for full instructions. Execute those instructions in full mode (sprint + recently updated tickets). Today is $TODAY. This is an automated background run — no interactive output needed, just sync the files and update brain.db." \
  >> "$LOG" 2>&1
